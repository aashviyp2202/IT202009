...
function getAccountBalance($account){
	$balance = 0;
        $stmt= $balance->prepare("SELECT  ifnull(sum(amount),0) from Transactions where act_src_id = :id";
        $r= $stmt->execute();
        if(!$r){
                echo var_export($stmt->errorInfo(), true);
                return -1;
...
function updateBalance($account){
	$balance=0;
        $stmt= $balance->prepare("UPDATE Account set balance = (SELECT IFNULL(SUM(amount),0) from Transactions where act_src_id = :id)";
        $r= $stmt->execute();
        if(!r){
               echo var_export($stmt->errorInfo(), true);
	       return -1;;
